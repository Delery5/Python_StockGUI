...
def import_stock_csv(stock_list):
    print("\nAdd historical data to the stock in the stock list")
    print("Stock List: [ ", end="")
    for stock in stock_list:
        print(stock.symbol, "",end="")
    print("]")
    ...
    for stock in stock_list:
        if stock.symbol == symbol:
            with open(filename, newline='') as stockdate:
                datareader = csv.reader(stockdate, delimiter=',')
                next(datareader)
